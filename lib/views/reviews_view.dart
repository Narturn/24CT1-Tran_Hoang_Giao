  ...
  Future<void> _confirmDeleteReview(ReviewModel review) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Xóa bài Review'),
        content: Text('Bạn có chắc muốn xóa bài review về "${review.targetName}" không?'),
        actions: [
    ...
    if (confirm == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Đã xóa bài review thành công.')),
        );
      }
    }
  }
                        ...
                        child: filteredReviews.isEmpty
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final canDelete = item.authorId == _db.uid || isAdmin;
                                  ...
                                  return Card(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          // Hiển thị Avatar, Khung, Tên & Danh hiệu trang bị của Tác giả + Nút Xóa
                                          Row(
                                            children: [
                                              Expanded(
                                              ...
                                              if (canDelete)
                                                IconButton(
                                                  ...
                                                  tooltip: isAdmin && item.authorId != _db.uid
                                                      ? 'Xóa review (Admin)'
                                                      : 'Xóa review của bạn',
                                                  onPressed: () => _confirmDeleteReview(item),
                                                ),
                                            ],
                                          ),
                                          const SizedBox(height: 8),
